...
const populateDb2 = () => {
	...
	csv.parseFile("CD 2.csv", { headers: true, skip_blanks: true
	})
	...
	let stream = fs.createReadStream("CD 2.csv")	
	let csvStream = csv
		...
		.on("end", function () {
			...
			connection.connect((error) => {
				...
				if (error) {
					...
					let dropTable = 'DROP TABLE IF EXISTS `CD 2`'
					let createTable = 'CREATE TABLE `CD 2`' + '(' + headersWithProperties + ')'
					
					let insertData = 'INSERT INTO `CD 2` ' + '(' + correctHeaderFormat + ') ' + 'VALUES ?'
					//drop table
