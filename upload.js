...
const populateDb1 = () => {
	...
	csv.parseFile("CD 1.csv", { headers: true, skip_blanks: true
	})
	...
	let stream = fs.createReadStream("CD 1.csv")	
	let csvStream = csv
		...
		.on("end", function () {
			...
			connection.connect((error) => {
				...
				if (error) {
					...
					let dropTable = 'DROP TABLE IF EXISTS `CD 1`'
